...
from nets.centernet_newmask import CenterNet_HourglassNet, CenterNet_Resnet50
from utils.utils import (cvtColor, get_classes, preprocess_input, resize_image,
                         show_config, letterbox_resize_back)
...
class CenterNet(object):
    _defaults = {
        ...
        "input_shape": [512, 512],
        # --------------------------------------------------------------------------#
        #   只有得分大于置信度的预测框会被保留下来，进行卵石簇预测时建议设置的置信度为0.2，可根据实际效果提高或降低
        # --------------------------------------------------------------------------#
        "confidence": 0.19,
        # ---------------------------------------------------------------------#
        #   非极大抑制所用到的nms_iou大小
    ...
    def detect_mask(self, image, alpha=0.5):
        ...
        if mask is None:
            print("Mask not available for this backbone")
            return image
        # ---------------------------------------------------#
        #  mask还原到原图大小
        # ---------------------------------------------------#
        mask_prob = torch.sigmoid(mask)[0, 0].cpu().numpy()
        mask_resized = cv2.resize(mask_prob, (self.input_shape[1], self.input_shape[0]), interpolation=cv2.INTER_LINEAR)
        mask_resized = letterbox_resize_back(mask_resized, image_shape, self.input_shape, self.letterbox_image)
        # ---------------------------------------------------#
        #  二值化
        # ---------------------------------------------------#
        ...
        overlay_np[binary_mask == 1] = (
                (1 - alpha) * image_np[binary_mask == 1] + alpha * red_layer[binary_mask == 1]
        ).astype(np.uint8)
