...
from nets.resnet50mask import resnet50FPN, resnet50_SEDecoder, resnet50_maskHead
...
class CenterNet_Resnet50(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("Backbone output shapes:", p2.shape, p3.shape, p4.shape, p5.shape)
        out = self.decoder(p3, p4, p5)
        #print("Decoder output shape:", out.shape)
        head_out = self.head(out)
        # if isinstance(head_out, tuple) or isinstance(head_out, list):
        #     for i, h in enumerate(head_out):
        #         print(f"Head output {i} shape:", h.shape)
        # else:
