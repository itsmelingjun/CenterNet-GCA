...
import torch
from nets.newmask_centernet_training import focal_loss, reg_l1_loss, mask_loss
...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen,
    ...
    for iteration, batch in enumerate(gen):
        ...
        batch_images, batch_hms, batch_whs, batch_regs, batch_reg_masks, batch_masks = batch
        # ----------------------#
        #   清零梯度
        # ----------------------#
        optimizer.zero_grad()
        if not fp16:
            if backbone == "resnet50":
                hm, wh, offset, mask = model_train(batch_images)
                c_loss = focal_loss(hm, batch_hms)
                wh_loss = 0.1 * reg_l1_loss(wh, batch_whs, batch_reg_masks)
                off_loss = reg_l1_loss(offset, batch_regs, batch_reg_masks)
                m_loss = mask_loss(mask, batch_masks)
        ...
        if local_rank == 0:
            pbar.set_postfix(**{'total_r_loss': total_r_loss / (iteration + 1),
                                ...
                                'total_m_loss': total_m_loss / (iteration + 1),
                                'lr': get_lr(optimizer)})
            pbar.update(1)
